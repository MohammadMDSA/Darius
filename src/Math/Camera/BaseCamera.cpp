...
namespace Darius::Math::Camera
{
	...
	void BaseCamera::Update()
	{
		...
		// Updating View Space Frustum
		if (mFrustumVSDirty)
		{
			m_FrustumVS = Frustum(m_ProjMatrix);
			mFrustumVSDirty = false;
			mFrustumWSDirty = true;
		}

		if (m_CameraToWorld != m_PrevCameraToWorld)
			mFrustumWSDirty = true;

		// Updating World Space Frustum
		if (mFrustumWSDirty)
		{
			m_FrustumWS = m_CameraToWorld * m_FrustumVS;
			mFrustumWSDirty = false;
		}

		m_PrevCameraToWorld = m_CameraToWorld;
	}
}
